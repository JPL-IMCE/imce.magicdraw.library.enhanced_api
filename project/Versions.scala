object Versions {

  // this project
  val version = "2.0"

  // https://github.jpl.nasa.gov/imce/jpl-mbee-common-scala-libraries
  val jpl_mbee_common_scala_libraries="1.11"

  // https://github.jpl.nasa.gov/mbee-dev/cae.magicdraw.package.mdk
  val mdk_package = "2.3-RC3"

  // https://cae-artifactory.jpl.nasa.gov/artifactory/simple/ext-release-local/com/nomagic/magicdraw/application/magicdraw
  val magicdraw_no_install = "18.0.sp5"

  // https://github.jpl.nasa.gov/imce/jpl-mbee-common-scala-libraries/blob/master/project/Versions.scala#L23
  val org_aspectj_version = "1.8.7"
}